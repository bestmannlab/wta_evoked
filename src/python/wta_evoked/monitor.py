...
import numpy as np
from brian import MultiStateMonitor, PopulationRateMonitor, SpikeMonitor, raster_plot, ms, hertz, nS, nA, defaultclock, StateMonitor, mA
import matplotlib.pyplot as plt

# Collection of monitors for WTA network
from wta_evoked.utils import get_response_time, plot_network_firing_rates, FitRT

class SessionMonitor():
    def __init__(self, network, sim_params, conv_window=10, record_firing_rates=False, record_lfp=False):
    ...
    def record_trial(self, trial_idx, inputs, correct_input, wta_monitor):
        ...
        if self.record_lfp:
            self.lfp.append(wta_monitor.monitors['lfp'][0]/mA)
    ...
    def plot(self):
        ...
        if self.record_lfp:
            ...
            for coherence in coherence_levels:
                ...
                coherence_lfps=[]
                for trial_idx in trials:
                    coherence_lfps.append(self.lfp[trial_idx])
                coherence_lfps=np.array(coherence_lfps)
                mean_coherence_lfps=np.mean(coherence_lfps,axis=0)
                stderr_coherence_lfps=np.std(coherence_lfps,axis=0)/np.sqrt(len(trials))
                coherence_means.append(mean_coherence_lfps)
                coherence_stderrs.append(stderr_coherence_lfps)
            coherence_means=np.array(coherence_means)
            coherence_stderrs=np.array(coherence_stderrs)
            max_lfp=np.max(coherence_means[:,500:])+np.max(coherence_stderrs[:,500:])
            min_lfp=np.min(coherence_means[:,500:])-np.max(coherence_stderrs[:,500:])
            rect=Rectangle((0,0),(self.sim_params.stim_end_time-self.sim_params.stim_start_time)/ms, max_lfp+5,
                alpha=0.25, facecolor='yellow', edgecolor='none')
            ax.add_patch(rect)
            for idx, coherence in enumerate(coherence_levels):
                time_ticks=(np.array(range(coherence_means.shape[1]))*self.sim_params.dt)/ms-self.sim_params.stim_start_time/ms
                baseline,=ax.plot(time_ticks, coherence_means[idx,:], label='Coherence=%.3f' % coherence)
                ax.fill_between(time_ticks, coherence_means[idx,:]-coherence_stderrs[idx,:], coherence_means[idx,:]+coherence_stderrs[idx,:], alpha=0.5,
                    facecolor=baseline.get_color())
            plt.ylim(min_lfp,max_lfp)
...
class WTAMonitor():
    ...
    def __init__(self, network, sim_params, lfp_source=None, record_neuron_state=False, record_spikes=True, record_firing_rate=True,
        ...
        self.record_lfp=lfp_source is not None
        self.save_summary_only=save_summary_only
